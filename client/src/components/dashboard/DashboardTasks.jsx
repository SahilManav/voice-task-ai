    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-violet-500/40 bg-gradient-to-r from-violet-500/15 to-purple-500/10 px-4 py-3 shadow-[0_0_20px_rgba(139,92,246,0.1)]">
          <div className="flex items-center gap-2.5">
            ...
            {pendingTasks.length > 0 && (
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-violet-500 text-[11px] font-bold text-gray-900 dark:text-white shadow-md">
                {pendingTasks.length}
              </span>
            )}
          </div>
      ...
      {isLoadingTasks ? (
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-[#141A29] border-gray-200 dark:border-white/5 p-5 space-y-3 animate-pulse">
      ...
      {showCompletedSection && completedTasks.length > 0 && (
        <div className="space-y-4 pt-4">
          <p className="text-xs font-bold uppercase text-gray-400 dark:text-gray-600">
